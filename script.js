...
window.loguj = async () => {
    ...
    if (error) {
        if(error.message.includes("confirm")) alert("Potwierdź e-mail! Sprawdź pocztę (również SPAM).");
...
window.zarejestruj = async () => {
    ...
    if(!nick || !email) return alert("Wypełnij nick i email!");
    if(!sprawdzHaslo(password)) return alert("Słabe hasło! Musi mieć 8 znaków, wielką literę, cyfrę i znak specjalny.");
    ...
    if (error) alert(error.message);
        ...
        alert("Wysłaliśmy link aktywacyjny na Twój e-mail. POTWIERDŹ GO, żeby móc się zalogować!");
        location.reload();
    }
};
...
window.pokazSzczegoly = async (id) => {
    ...
    <button onclick="wyslijWiadomosc('${o.email_autora}')" style="width:100%; padding:15px; border-radius:10px; border:none; background:#111; color:white; font-weight:800; cursor:pointer">Napisz</button>` 
    : `<div style="color:red; font-weight:800; text-align:center; padding:20px">Zaloguj się, aby zobaczyć kontakt</div>`;
